import { Component, OnDestroy, OnInit, ViewEncapsulation, Compiler } from '@angular/core';
...
export class ChangePasswordComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (retrievedObject) {
      ...
      if (this.authenticationDetails.userType === 'Himalaya email id user') {
        this.notificationSnackBarComponent.openSnackBar('This option is only for Non Himalaya email id user', SnackBarStatus.danger);
      ...
      if (this.changePassword.CurrentPassword === this.changePassword.NewPassword) {
        ...
        this._authService.ChangePassword(this.changePassword).subscribe(
          (data) => {
            this.ResetControl();
            this.IsProgressBarVisibile = false;
            this.notificationSnackBarComponent.openSnackBar('Password changed successfully, please log in with new password', SnackBarStatus.success);
